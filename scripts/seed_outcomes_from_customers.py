...
Com --batch-id, processa apenas predições do ciclo de avaliação especificado.
Sem --batch-id, processa todas as predições holdout sem outcome (comportamento original).
...
_QUERY_CANDIDATES_BY_BATCH = text("""
    SELECT
        p.id          AS prediction_id,
        p.tenant_id,
        p.project_id,
...
def seed_outcomes(
    ...
    Com batch_id, filtra apenas predições do ciclo de avaliação especificado.
    ...
    with engine.connect() as conn:
        ...
        params: dict = {"tenant_id": tenant_id, "project_id": project_id}
        if batch_id:
            query = _QUERY_CANDIDATES_BY_BATCH
            params["batch_id"] = batch_id
        else:
            query = _QUERY_CANDIDATES
        rows = conn.execute(query, params).fetchall()

    logger.info(
        "outcomes_candidates_found",
        count=len(rows),
        tenant=tenant_slug,
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "Se fornecido, semeia apenas outcomes deste batch.",
    )
