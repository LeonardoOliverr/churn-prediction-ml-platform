...
def _call(engine, raise_on_execute=False):
    if raise_on_execute:
        engine.begin.return_value.__enter__.return_value.execute.side_effect = Exception(
            "DB offline"
        )
    log_prediction_bg(
        engine=engine,
...
def test_log_prediction_bg_swallows_exception():
    ...
    engine = MagicMock()
    engine.begin.return_value.__enter__ = MagicMock(side_effect=Exception("DB offline"))
    engine.begin.return_value.__exit__ = MagicMock(return_value=False)
