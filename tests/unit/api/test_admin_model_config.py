...
def test_validate_model_for_project_accepts_global_approved_model():
    ...
    model = {"id": "model-1", "tenant_id": None, "project_id": None, "scope": "global", "status": "approved"}
...
def test_validate_model_for_project_rejects_not_approved_model():
    ...
    project = {"id": "project-1", "tenant_id": "tenant-1"}
    model = {"id": "model-1", "tenant_id": None, "project_id": None, "scope": "global", "status": "validated"}
...
def test_configure_challenger_rejects_active_champion_as_challenger():
    ...
    with patch("src.routers.admin._get_project", return_value={"id": "project-1", "tenant_id": "tenant-1"}), \
         patch("src.routers.admin._validate_model_for_scope"), \
         patch("src.routers.admin._active_config_by_model", return_value={"id": "cfg-1", "role": "champion"}):
        with pytest.raises(HTTPException) as exc:
