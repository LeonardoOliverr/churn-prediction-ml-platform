...
def test_promotion_champion_is_never_candidate():
    ...
    result = _compute_promotion_recommendation(
        metrics=_metrics(f1=0.9),
        cost=1000.0,
        total=500,
        role="champion",
        champion_metrics=_metrics(f1=0.8),
        champion_cost=1200.0,
        champion_total=500,
    )
...
def test_promotion_challenger_better_f1_and_cost():
    ...
    result = _compute_promotion_recommendation(
        metrics=_metrics(f1=0.85),
        cost=200.0,
        total=200,
        role="challenger",
        champion_metrics=_metrics(f1=0.80),
        champion_cost=1000.0,
        champion_total=500,
    )
...
def test_promotion_champion_superior_in_both():
    ...
    result = _compute_promotion_recommendation(
        metrics=_metrics(f1=0.70),
        cost=1800.0,
        total=200,
        role="challenger",
        champion_metrics=_metrics(f1=0.85),
        champion_cost=1000.0,
        champion_total=500,
    )
    assert result["promotion_candidate"] is False
...
def test_compute_row_cost_flat_config_returns_flat_values():
    ...
    config = {
        ...
        "fp_cost_months": 3,
        "fp_cost_discount": 0.5,
        "fn_cost_cltv_multiplier": 2.0,
        "fn_cost_months_multiplier": 6,
    }
...
def test_compute_row_cost_monthly_charges_model():
    ...
    config = {
        ...
        "fp_cost_months": 1.0,
        "fp_cost_discount": 1.0,
        "fn_cost_cltv_multiplier": 2.0,
        "fn_cost_months_multiplier": 12,
    }
...
def _pred_row(pred: bool, churned: bool, monthly: float = 50.0, cltv: float = 1000.0):
    return SimpleNamespace(churn_pred=pred, churned=churned, monthly_charges=monthly, cltv=cltv)
...
def test_compute_cost_weighted_flat_no_config():
    ...
    total, avg_fp, avg_fn = _compute_cost_weighted(
        rows, None, fp_cost_flat=100.0, fn_cost_flat=2000.0
    )
...
def test_evaluate_returns_empty_when_no_rows():
    ...
    with (
        patch("ml.evaluate_production._build_engine", return_value=engine),
