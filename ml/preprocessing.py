"""
Carregamento de dados do PostgreSQL e pipeline de pré-processamento reutilizável.
"""
...
def load_data(
    ...
    """Carrega churn.customers do PostgreSQL e retorna (X, y) prontos para sklearn.

    Escopos:
        global  — ambos None            → todos os tenants (sem filtro)
        tenant  — só tenant_slug        → clientes desse tenant
        project — tenant_slug + project_slug → clientes desse projeto
    """
    ...
    # Booleans PostgreSQL chegam como object em alguns drivers — normaliza para float
    for col in BOOL_FEATURES:
        df[col] = pd.to_numeric(df[col].map({True: 1, False: 0, 1: 1, 0: 0}), errors="coerce")
...
def build_preprocessor() -> ColumnTransformer:
    """Retorna ColumnTransformer com imputation, scaling e encoding configurados."""
